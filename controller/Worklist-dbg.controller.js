		...
		onCreateObject: function (oControl, aControls, iKey) {
			if (oControl.type === "l") {
				...
				if (oControl.subType === "i") {
					...
					if (oControl.source.includes('Eps')) {
						var oItemTemplate = new sap.ui.core.Item({
							text: '{EpsDescripcion}',
							key: '{EpsCodigo}'
						});
						comboBox.setModel(aEPSs);
					} else if (oControl.source.includes('tipoDoc')) {
						var oItemTemplate = new sap.ui.core.Item({
							text: '{TipoDocumentoDescripcion}',
							key: '{TipoDocumentoCodigo}'
						});
						comboBox.setModel(aTipoDoc);
					}
			...
			var oTiles = {
				"TileCollection": [
					...
					// }
					{
						...
						//https://colsubsidiojobappb8sppjv7s2.us3.hana.ondemand.com/colsubsidiojob-app/dispensacion/sync/interlocutores/0800088702/13/18776346
						"icon": "sap-icon://database",
						"title": "Sincronizacion usuario",
						"info": "Habilitado",
						"infoState": "Success",
						"method": "GET",
						"url": "dispensacion/sync/interlocutores/",
						"param": [{
							"grupo": [{
								type: "l",
								text: "EPS",
